...
*/
/*
TEST_F(SkipTest, UniRandForTest) { // Skiplist test for Random Pattern - Signal.Jin
  const int N = 1000000; // Write Count - Signal.Jin
  //const int R = 100000000; // Read Count - Signal.Jin
  Random rnd(5326);
  std::set<Key> keys;
  Arena arena;
  TestComparator cmp;
  ...
  std::random_device rd;
  std::mt19937 gen(rd());
  std::uniform_int_distribution<int> distr(0, N*3);
  ...
  for (int i = 0; i < 100; i++) { 
    ...
    for (j = 0; j < 1000000; j++) {
      Key Gkey = distr(gen);
      if (list.Contains_RB(Gkey)) { // Maybe estimate time in here - Signal.Jin
        ASSERT_EQ(keys.count(Gkey), 1U);
      } else {
        ASSERT_EQ(keys.count(Gkey), 0U);
      }
...
TEST_F(SkipTest, YCSBWorkloadE) { // Skiplist test for Random Pattern - Signal.Jin
  ...
  for (int i = 0; i < run_cnt; i++) {
    ...
    if (next_op < 95) {
      if (list.Contains_TreeScan(Zkey, 100)) {
        ASSERT_EQ(keys.count(Zkey), 1U);
      } else {
        ASSERT_EQ(keys.count(Zkey), 0U);
      }
...
TEST_F(SkipTest, UniRandInsertAndLookup) { // Skiplist test for Random Pattern - Signal.Jin
  const int N = 16000000; // Write Count - Signal.Jin
  const int R = 16000000; // Read Count - Signal.Jin
  Random rnd(5326);
  std::set<Key> keys;
  Arena arena;
  TestComparator cmp;
