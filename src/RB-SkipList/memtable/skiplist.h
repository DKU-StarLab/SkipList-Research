...
*/
  ...
  bool Contains_TreeScan(const Key& key, uint64_t scan_num) const;
...
*/
  ...
  Node* FindGreaterOrEqual_TreeScan(const Key& key, uint64_t scan_num) const;
...
template<typename Key, class Comparator>
typename SkipList<Key, Comparator>::Node* SkipList<Key, Comparator>::
  FindGreaterOrEqual_TreeScan(const Key& key, uint64_t scan_num) const {
  // Note: It looks like we could reduce duplication by implementing
  // this function as FindLessThan(key)->Next(0), but we wouldn't be able
  // to exit early on equality and the result wouldn't even be correct.
  // A concurrent insert might occur after FindLessThan(key) but before
...
template<typename Key, class Comparator>
bool SkipList<Key, Comparator>::Contains_TreeScan(const Key& key, uint64_t scan_num) const {
  Node* x = FindGreaterOrEqual_TreeScan(key, scan_num);
  if (x != nullptr && Equal(key, x->key)) {
    return true;
  } else {
    return false;
