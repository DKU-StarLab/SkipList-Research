...
class InlineSkipList {
 ...
 public:
  ...
  bool Insert_RB(const char* key); // Signal.Jin
  ...
  template <bool UseCAS>
  bool Insert_RB(const char* key, Splice* splice, bool allow_partial_splice_fix); // Signal.Jin
 ...
 private:
  ...
  mutable RBnode* rootRB; // Signal.Jin - root of RB-Tree
  mutable RBnode* leafNode; // Signal.Jin - leaf node of RB-Tree
  ...
  // Signal.Jin - RB-Tree Functions
  void InsertRBTree(const char* key, Node* sl_node) const;
  void InsertFixUp(RBnode* z) const;
  void RotateL(RBnode* x) const;
  void RotateR(RBnode* y) const;
  Node* SearchRBNode(const char* key) const;
...
template <class Comparator>
bool InlineSkipList<Comparator>::Insert_RB(const char* key) {
  Node* prev[kMaxPossibleHeight];
  Node* next[kMaxPossibleHeight];
  Splice splice;
  splice.prev_ = prev;
  splice.next_ = next;
  return Insert_RB<true>(key, &splice, false);
} // Signal.Jin
...
template <class Comparator>
template <bool UseCAS>
bool InlineSkipList<Comparator>::Insert_RB(const char* key, Splice* splice,
                                        bool allow_partial_splice_fix) {
  Node* x = reinterpret_cast<Node*>(const_cast<char*>(key)) - 1;
  const DecodedKey key_decoded = compare_.decode_key(key);
  int height = x->UnstashHeight();
  ...
  if (splice->height_ < max_height) {
    ...
    if (temp == head_) {
      ...
      assert(max_height == kMaxHeight_);
      splice->prev_[max_height] = temp;
      splice->next_[max_height] = temp->Next(kMaxHeight_);
    }
  ...
  if (height == kMaxHeight_) {
    InsertRBTree(x->Key(), x);
  } // Signal.Jin

  return true;
} // Signal.Jin - Insert function (SkipList with RB-Tree)
...
// Signal.Jin - RB-Tree Function Implementation
template<class Comparator>
void InlineSkipList<Comparator>::
InsertRBTree(const char* key, Node* sl_node) const {
  RBnode* x = rootRB;
  RBnode* y = nullptr;
  RBnode* z = new RBnode();
...
template<class Comparator>
typename InlineSkipList<Comparator>::Node* InlineSkipList<Comparator>::
  SearchRBNode(const char* key) const {
  RBnode* pos = rootRB;
