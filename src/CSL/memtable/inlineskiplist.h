...
class InlineSkipList {
 ...
 public:
  ...
  // Signal.Jin - Insert function for Compact-Skiplist
  bool InsertConcurrently_Array(const char* key);
  ...
  // Signal.Jin
  template <bool UseCAS>
  bool Insert_Array(const char* key, Splice* splice, bool allow_partial_splice_fix);

  // Returns true iff an entry that compares equal to key is in the list.
  bool Contains(const char* key) const;

  // Signal.Jin
  bool Contains_Array(const char* key);

  // Return estimated number of entries smaller than `key`.
  uint64_t EstimateCount(const char* key) const;
  uint64_t Estimate() const; // Signal.Jin
  ...
  class Iterator {
   public:
    ...
    // Signal.Jin 
    void Seek_Array(const char* target);
    uint64_t key_array(uint64_t key) const;
 ...
 private:
  ...
  //Signal.Jin
  Node* FindGreaterOrEqual_Array(const char* key);
...
struct InlineSkipList<Comparator>::Node {
  ...
  const char* Key() const { return reinterpret_cast<const char*>(&next_[1]+2); }
 ...
 public:
  // Compact-skiplist essential parameter - Signal.Jin
  uint64_t array_size;
  uint64_t *array;
};
...
inline void InlineSkipList<Comparator>::Iterator::SetList(
    const InlineSkipList* list) {
  list_ = list;
  list_array = const_cast<InlineSkipList*>(list);
  node_ = nullptr;
}
...
// Signal.Jin
template <class Comparator>
inline uint64_t InlineSkipList<Comparator>::Iterator::key_array(uint64_t key) const {
  assert(Valid());
  uint64_t founded_key;
  printf("node->array_size=%lu, key=%lu\n", node_->array_size, key);

  uint64_t fake_key;
  memcpy(&fake_key, node_->Key(), sizeof(uint64_t));
  printf("fake key = %lu\n", fake_key);

  if (node_ != nullptr) {
    uint64_t pos = (key & (MOD-1)) / (MOD/(node_->array_size));
    founded_key = node_->array[pos];
    printf("pos = %lu\n", pos);
  }
  printf("founded_key = %lu\n", founded_key);
  return founded_key;
}
...
// Signal.Jin
template <class Comparator>
inline void InlineSkipList<Comparator>::Iterator::Seek_Array(const char* target) {
  node_ = list_array->FindGreaterOrEqual_Array(target);
}
...
template <class Comparator>
typename InlineSkipList<Comparator>::Node*
InlineSkipList<Comparator>::FindGreaterOrEqual_Array(const char* key){
  // Note: It looks like we could reduce duplication by implementing
  // this function as FindLessThan(key)->Next(0), but we wouldn't be able
  // to exit early on equality and the result wouldn't even be correct.
  // A concurrent insert might occur after FindLessThan(key) but before
  // we get a chance to call Next(0).
  uint64_t r_key;
  memcpy(&r_key, key, sizeof(uint64_t));

  uint64_t quot = r_key >> (31 - __builtin_clz(MOD));

  char* f_key = AllocateKey(sizeof(uint64_t));
  memcpy(f_key, &quot, sizeof(uint64_t));

  const char* fake_key = f_key;

  //printf("r_key = %lu, quot = %lu\n", r_key, quot);

  Node* x = head_;
  int level = GetMaxHeight() - 1;
  Node* last_bigger = nullptr;
  const DecodedKey key_decoded = compare_.decode_key(fake_key);
  while (true) {
    Node* next = x->Next(level);
    if (next != nullptr) {
      PREFETCH(next->Next(level), 0, 1);
...
template <class Comparator>
uint64_t InlineSkipList<Comparator>::Estimate() const {
  uint64_t node;
  ...
  while (true) {
    Node* next = x->Next(level);
    if (next != nullptr) {
      memcpy(&node, next->Key(), sizeof(uint64_t));
      printf("%lu ", node);
    } else if (next == nullptr) {
      level--;
      printf("\n");
    } else if (level == 0 && next == nullptr) {
      break;
    }
  }
  return node;
...
// Signal.Jin
template <class Comparator>
bool InlineSkipList<Comparator>::InsertConcurrently_Array(const char* key) {
  Node* prev[kMaxPossibleHeight];
  Node* next[kMaxPossibleHeight];
  Splice splice;
  splice.prev_ = prev;
  splice.next_ = next;
  return Insert_Array<true>(key, &splice, false);
}
...
void InlineSkipList<Comparator>::FindSpliceForLevel(const DecodedKey& key,
  ...
  while (true) {
    ...
    /*if (before != nullptr) {
        memcpy(&prev_key, before->Key(), sizeof(uint64_t));
        printf("prev = %lu\n", prev_key);
    }
    if (next != nullptr) {
      memcpy(&next_key, next->Key(), sizeof(uint64_t));
      printf("next = %lu\n", next_key);
    }*/
    if (next != nullptr) {
      PREFETCH(next->Next(level), 0, 1);
...
template <class Comparator>
template <bool UseCAS>
bool InlineSkipList<Comparator>::Insert_Array(const char* key, Splice* splice,
                                        bool allow_partial_splice_fix) {
  // In here, we can extract integer key data from (char* key) - Signal.Jin
  uint64_t r_key;
  memcpy(&r_key, key, sizeof(uint64_t));

  //uint64_t quot = r_key >> (31 - __builtin_clz(MOD));
  uint64_t quot = r_key >> (uint64_t)(log2(MOD));
  uint64_t remain = r_key & (MOD - 1);
  ...
  Node* x = reinterpret_cast<Node*>(const_cast<char*>(fake_key)) - 1;

  //printf("x->array=%p, x->array_size=%p, next_[0]=%p, next_[1]=%p\n", &x->array, &x->array_size, &x->next_[0], &x->next_[1]);
  ...
  {
    std::lock_guard<std::mutex> lock1(mtx1); // lock for update array size - Signal.Jin
    if (splice != nullptr) {
      if (splice->next_[0] != nullptr && Equal(splice->next_[0]->Key(), x->Key())) {
        uint64_t pos = remain >> (31 - __builtin_clz(MOD >> (31 - __builtin_clz(splice->next_[0]->array_size))));
        if (splice->next_[0]->array[pos] == 0) {
          splice->next_[0]->array[pos] = r_key;
        } else if (splice->next_[0]->array[pos] == r_key) {
          //return false;
        } else {
          uint64_t re_array_size = MOD >> (uint64_t)floor(log2(abs((int)splice->next_[0]->array[pos] - (int)r_key)));
          uint64_t div_shift_bit = 31 - __builtin_clz(MOD >> (31 - __builtin_clz(re_array_size)));

          uint64_t *buf2 = new uint64_t[re_array_size]();
          for(uint64_t j = 0; j < splice->next_[0]->array_size; j++) {
            if (splice->next_[0]->array[j] != 0) {
              uint64_t pos2 = (splice->next_[0]->array[j] & (MOD-1)) >> div_shift_bit;
              buf2[pos2] = splice->next_[0]->array[j];
            }
          }
          splice->next_[0]->array_size = re_array_size;
          delete [] splice->next_[0]->array;
          splice->next_[0]->array = buf2;
  ...
  { // Initiate compact-array - Signal.Jin
    std::lock_guard<std::mutex> lock2(mtx2); // lock for insert new node - Signal.Jin

    x->array_size = 4;
    uint64_t *buf3 = new uint64_t[x->array_size]();
    int64_t pos = remain / (MOD/x->array_size);
    buf3[pos] = r_key;
    x->array = buf3;
  }
  ...
  if (splice_is_valid) {
   // printf("valid\n");
    for (int i = 0; i < height; ++i) {
      //uint64_t test_key;
      splice->prev_[i] = x;
      //memcpy(&test_key, x->Key(), sizeof(uint64_t));
      //printf("x=%lu\n", test_key);
    }
    assert(splice->prev_[splice->height_] == head_);
    assert(splice->next_[splice->height_] == nullptr);
    for (int i = 0; i < splice->height_; ++i) {
      assert(splice->next_[i] == nullptr ||
             compare_(fake_key, splice->next_[i]->Key()) < 0);
      assert(splice->prev_[i] == head_ ||
             compare_(splice->prev_[i]->Key(), fake_key) <= 0);
      assert(splice->prev_[i + 1] == splice->prev_[i] ||
             splice->prev_[i + 1] == head_ ||
             compare_(splice->prev_[i + 1]->Key(), splice->prev_[i]->Key()) <
                 0);
...
template <class Comparator>
bool InlineSkipList<Comparator>::Contains_Array(const char* key) {
  Node* x = FindGreaterOrEqual_Array(key);
  uint64_t res = 0;
  ...
  if (x != nullptr) {
    uint64_t pos = (r_key & (MOD-1)) / (MOD/(x->array_size));
    res = x->array[pos];
  }

  char* f_key = AllocateKey(sizeof(uint64_t));
  memcpy(f_key, &res, sizeof(uint64_t));

  const char* found_key = f_key;

  //printf("%lu, %lu\n", r_key, res);

  if (x != nullptr && Equal(key, found_key)) {
    return true;
  } else {
    return false;
  }
